...
public double getInterest(double aIR,double iIA,double nOY){
  return iIA * Math.pow(( 1 + aIR/12 ), 12*nOY);
}
  class CalculateAmount implements EventHandler<ActionEvent>{
    public void handle(ActionEvent e){
      i = Float.parseFloat(t1.getText());
      p = Float.parseFloat(t2.getText());
      n = Float.parseFloat(t3.getText());
      t4.setText("" + getInterest(i,p,n));
    }
  }
}